...
## 5. Decision — Inertia/Vue ทั้ง 2 ฝั่ง (ไม่ใช้ Filament)

> อัปเดต 2026-06-29 ตาม `massage-member-system-spec.md`: **ตัด Filament ออก** — spec กำหนด admin เป็น Inertia/Vue (`Pages/Admin/`, `AdminLayout.vue`) ด้วย เพราะหัวใจคือ redemption console ที่เป็น UX custom + stack เดียว guard/layout เดียว ไม่ต้องมี auth layer ของ Filament มาชน 2-guard (member/admin)

**บริบทที่ใช้ตัดสิน:**
- ระบบสมาชิกร้านนวด — หัวใจ = แพ็ค/ตัดสิทธิ์ (entitlement), CRUD catalog ไม่เยอะจนต้องพึ่ง Filament
- เจ้าของอยาก **เรียน Vue** (เลือก **Vue** มากกว่า React — ใกล้ Blade/Alpine)

**สแตกที่เลือก — Laravel 13 แอปเดียว, Inertia/Vue ทั้งคู่:**

| ส่วน | สแตก | เหตุผล |
|---|---|---|
| หน้าบ้าน (Member) | **Inertia 3 + Vue 3 + TS + Tailwind 4** — `Pages/Member/`, `MemberLayout.vue` | สวย+animation, login LINE |
| หลังบ้าน (Admin) | **Inertia 3 + Vue 3 + TS + Tailwind 4** — `Pages/Admin/`, `AdminLayout.vue` | เน้นเร็ว, redemption console custom, email+password+role |

> 2 guard แยกชัด: `members` (LINE) / `users` (admin email+password role owner\|staff) — ดู `docs/architecture.md`

**แผนเริ่ม:**
1. `docker compose exec workspace bash docker/scaffold.sh` → Laravel 13 + Vue starter kit + MariaDB
2. Phase 1 migration (entitlement schema) หลัง review ERD — ดู `docs/architecture.md`
3. Design tokens — ดู `docs/design-system.md`

**สถานะ:** _Filament ตัดออกแล้ว · Phase 0 scaffold พร้อมรัน · Phase 1 ERD รอ review_
